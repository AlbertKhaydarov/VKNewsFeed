...
class ___VARIABLE_sceneName___InteractorTests: XCTestCase {
  ...
  class ___VARIABLE_sceneName___PresentationLogicSpy: ___VARIABLE_sceneName___PresentationLogic {
    ...
    func presentData(response: ___VARIABLE_sceneName___.Model.Response) {
  ...
  func testMakeRequest() {
    ...
    // When
    sut.makeRequest(request: request)
    
    // Then
    XCTAssertTrue(spy.presentDataCalled, "makeRequest(request:) should ask the presenter to format the result")
  }
