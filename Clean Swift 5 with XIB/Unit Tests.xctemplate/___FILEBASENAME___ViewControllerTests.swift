...
class ___VARIABLE_sceneName___ViewControllerTests: XCTestCase {
  ...
  class ___VARIABLE_sceneName___BusinessLogicSpy: ___VARIABLE_sceneName___BusinessLogic {
    ...
    func makeRequest(request: ___VARIABLE_sceneName___.Model.Request) {
  ...
  func testShouldmakeRequestWhenViewIsLoaded() {
    ...
    // Then
    XCTAssertTrue(spy.makeRequestCalled, "viewDidLoad() should ask the interactor to do Model")
  }
  ...
  func testDisplayData() {
    ...
    // Then
    //XCTAssertEqual(sut.nameTextField.text, "", "displayData(viewModel:) should update the name text field")
  }
